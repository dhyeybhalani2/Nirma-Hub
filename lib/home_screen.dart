...
class _HomePageState extends ConsumerState<HomePage> with WidgetsBindingObserver {
                            ...
                            children: [
                              SizedBox(
                                ...
                                child: PageView(
                                  ...
                                  children: [
                                    RepaintBoundary(
                                      child: _buildTimetableCard(allEvents),
                                    ),
                                    RepaintBoundary(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: _buildCoreToolCard(
                                  ...
                                  onTap: () => Navigator.push(
                                    context,
                                    _createRoute(const SgpaCalculatorScreen()),
                                  ),
                                ),
                              ),
                            ],
  ...
  Widget _buildTimetableCard(List<EventModel> allEvents) {
    ...
    final todayHoliday = getHolidayForDate(now, allEvents);
    final bool isTodayHoliday = todayHoliday != null;

    bool advanceFromToday = isTodayHoliday ||
        now.weekday == DateTime.saturday ||
        now.weekday == DateTime.sunday ||
        (todaysLectures.isNotEmpty && currentMins >= maxEndMins + 30) ||
        (!isTimetableLoading && todaysLectures.isEmpty && currentMins > 12 * 60);
    ...
    if (advanceFromToday) {
      ...
      while (safetyCounter < 30 &&
          (targetDate.weekday == DateTime.saturday ||
           ...
           getHolidayForDate(targetDate, allEvents) != null)) {
        targetDate = targetDate.add(const Duration(days: 1));
        safetyCounter++;
      }
    }

    final bool isTomorrowActive = (targetDate.year != now.year ||
