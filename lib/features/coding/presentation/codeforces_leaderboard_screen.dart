...
class CodeforcesLeaderboardScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CodeforcesLeaderboardScreen> createState() => _CodeforcesLeaderboardScreenState();
}

class _CodeforcesLeaderboardScreenState extends ConsumerState<CodeforcesLeaderboardScreen> {
  ...
  Future<void> _checkAndShowPromo() async {
    ...
    final hasShown = prefs.getBool('has_shown_codeforces_promo') ?? false;
    
    if (!hasShown && mounted) {
      await prefs.setBool('has_shown_codeforces_promo', true);
      Future.delayed(const Duration(seconds: 2), () {
        if (mounted) {
          LinkAccountPromoSheet.show(
            context,
            platform: 'Codeforces',
            primaryColor: const Color(0xFFEF4444), // Codeforces Red
  ...
  Future<void> _fetchLeaderboard() async {
    ...
    final data = await _codingService.getCodeforcesLeaderboard();
  ...
  void _showLinkDialog() async {
    ...
    final isLinked = currentCodingProfile != null && currentCodingProfile.codeforcesUsername != null;
    ...
    if (isLinked) {
      final shouldUnlink = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
            'Your Codeforces account is linked as:\n\n${currentCodingProfile!.codeforcesUsername}\n\nDo you want to unlink it?',
      ...
      if (shouldUnlink == true) {
        setState(() => _isLoading = true);
        final success = await _codingService.unlinkCodeforces(userProfile);
        if (success) {
          _fetchLeaderboard();
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Codeforces profile unlinked successfully!')),
            );
          }
        } else {
      ...
      final result = await showDialog<bool>(
        context: context,
        builder: (context) => const LinkProfileDialog(isLeetCode: false),
      );
      if (result == true) {
        _fetchLeaderboard();
        if (mounted) {
  ...
  List<CodingProfile> get _rankedProfiles {
    ...
    filtered.sort((a, b) {
      return b.codeforcesRating.compareTo(a.codeforcesRating);
    });
  ...
  Widget build(BuildContext context) {
    ...
    final searched = query.isEmpty 
        ...
        : ranked.where((profile) {
            ...
            final username = (profile.codeforcesUsername ?? '').toLowerCase();
  ...
  Widget _buildProfileCard(CodingProfile profile, int rank, {bool isCurrentUser = false, bool isPinned = false}) {
    return PremiumTouchButton(
      onTap: () async {
        final url = Uri.parse('https://codeforces.com/profile/${profile.codeforcesUsername}');
